classdef VideoReaderFFMPEG16bit < handle
   ...
   %
   %  CONSTRUCTOR:
   %     vr = VideoReaderFFMPEG16bit(fileName, 'tempFolder', tempFolder, 'FFMPEGPath', FFMPEGPath, 'imageFormat', imageFormat, 'verbose', verbose);
   %
   %     PARAMS:
   ...
   properties
      ...
      bufferSize
      bufferedFrameTimes
      tempName                % name for temporary tiff files - to avoid buffer collision if multiple instances of VideoReaderFFMPEG16bit access the same tempFolder
      % unused - delete or keep for compatibility?
      vr
   end
   
   methods (Access='public')
      function obj = VideoReaderFFMPEG16bit(vFileName, varargin)
         ...
         validatestring(p.Results.imageFormat,validFormats, 'VideoReaderFFMPEG16bit','imageFormat');
      ...
      function frame = read(obj, frameNumber)
         ...
         if length(frameNumber)==1
            ...
            frame = zeros(obj.Height,obj.Width,obj.Channels,length(frameTimes),'uint16');
